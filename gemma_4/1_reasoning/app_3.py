...
AUDIO_PATH = "/Users/preenav/Naveen Folder/Code Lab/Agents/ollamav1/gemma_4/1_reasoning/voice_note.wav"
...
def route_request(state: State) -> str:
    ...
    if any(keyword in user_text for keyword in ["audio", "voice", "voice note", "transcribe", "transcription"]):
        return "voice"
    if (
...
def get_whisper_model():
    try:
        ...
        raise SystemExit(
            "Missing Whisper dependency. Install requirements with "
            "`pip install -r requirements.txt` before running app_3.py."
        ) from exc
...
def transcribe_audio(audio_path: str) -> str:
    # Prepare audio at 16kHz mono and pass the waveform directly to Whisper.
    audio, _ = librosa.load(audio_path, sr=16000, mono=True)
    model = get_whisper_model()
    result = model.transcribe(audio, fp16=False)
    return result["text"].strip()

def voice_command_node(state: State):
    if not os.path.exists(AUDIO_PATH):
        raise SystemExit(f"Audio file not found: {AUDIO_PATH}")

    transcript = transcribe_audio(AUDIO_PATH)
    user_text = get_latest_user_text(state) or "Transcribe this voice note and identify the inventory action."
    print("\n--- WHISPER TRANSCRIPT ---")
    ...
    sys_msg = SystemMessage(
        content=(
            ...
            "Given a transcribed voice note, identify the inventory action required. "
            "Respond as compact JSON with keys transcript, action, item, quantity, and notes."
        )
    )
    follow_up = HumanMessage(
        content=(
            f"Original request: {user_text}\n"
            f"Transcribed voice note: {transcript}"
        )
    )
    response = llm.invoke([sys_msg, follow_up])
...
builder.add_node("voice", voice_command_node)
...
if __name__ == "__main__":
    ...
    initial_state = {
        "messages": [
            HumanMessage(content="Transcribe this voice note and identify the inventory action.")
        ]
    }
